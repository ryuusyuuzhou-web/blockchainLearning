...
console.log("\n1. 创建HD钱包")
// 通过助记词生成HD钱包
const mnemonic = `air organ twist rule prison symptom jazz cheap rather dizzy verb glare jeans orbit liu universe require tired sing casino business anxiety seminar hunt`
...
const amounts = Array(20).fill(ethers.parseEther("0.0000001"))
...
const addressAirdrop = config.ERC20.HOLESKY_WETH // Airdrop Contract
// 声明Airdrop合约
...
// WETH合约地址（Goerli测试网）
const addressWETH = config.ERC20.HOLESKY_WETH // WETH Contract
// 声明WETH合约
...
const main = async () => {
    ...
    if(ethers.formatEther(myETH) > 0.002 && ethers.formatEther(myToken) >= 0.002){
        ...
        // 发起交易
        const tx = await contractAirdrop.multiTransferETH(addresses, amounts, {value: ethers.parseEther("0.000002")})
        // 等待交易上链
        await tx.wait()
        // console.log(`交易详情：`)
        // console.log(tx)
